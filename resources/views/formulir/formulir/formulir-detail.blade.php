                ...
                <div class="box">
                    ...
                    <div class="box-body">
                        <div class="table-responsive">
                            <table class="table table-bordered" width="100%" cellspacing="0">
                                <tbody>
                                    <tr>
                                        @php
                                            switch ($data->komitmen_mr->verifikasi) {
                                                case '0':
                                                    $text = 'Draft';
                                                    break;
                        ...
                        <div id="komitmen">
                            ...
                            <div class="col-lg-12">
                                <label for="">1. Sasaran Program/Kegiatan Unit Pemilik
                                    Risiko</label>
                            </div>
                            <div class="col-lg-12">
                                ...
                                <table class="display table table-bordered table-hover" width="100%">
                                    <thead>
                                        <tr>
                                            <th>No</th>
                                            @if ($data->komitmen_mr->level == 'UPR-T1' ||
                                    ...
                                    <tbody>
                                        @if ($data->komitmen_mr->level == 'UPR-T3' || $data->komitmen_mr->level == 'PPK')
                                            @foreach ($data->komitmen_mr->sasaran_t3 as $item)
                                                <tr>
                                                    <td>{{ $data->komitmen_mr->level }}</td>
                                                    <td>{{ $item->SASARAN }}</td>
                                                    @if ($data->komitmen_mr->level == 'UPR-T3')
                                                        <td>{{ $item->isp_text . ' ' . number_format($item->isp_target, 3, ',', '.') . ' ' . $item->isp_satuan }}
                                                        </td>
                                                    @else
                                                        <td>{{ $item->INDIKATOR }}</td>
                                                    @endif
                                                    <td>{{ $item->nmpaket }}</td>
                                                    <td>{{ $item->kegiatan_tujuan . ' ' . number_format($item->tujuan_kegiatan, 3, ',', '.') . ' ' . $item->tujuan_kegiatan_satuan }}
                                                    </td>
                                                </tr>
                                            @endforeach
                                        @elseif($data->komitmen_mr->level == 'UPR-T2' && $data->komitmen_mr->creator->unit != 'Balai')
                                            @foreach ($data->komitmen_mr->sasaran_t3 as $item)
                                                <tr>
                                                    <td>{{ $data->komitmen_mr->level }}</td>
                                                    <td>{{ $item->SASARAN }}</td>
                                                    ...
                                                    @else
                                                        <td>{{ $item->INDIKATOR }}</td>
                                                    @endif
                                                    <td>{{ $item->kode_kegiatan . ' - ' . $item->NAMA }}</td>
                                                    <td>{{ $item->kegiatan_tujuan . ' ' . number_format($item->tujuan_kegiatan, 3, ',', '.') . ' ' . $item->tujuan_kegiatan_satuan }}
                                                    </td>
                                                </tr>
                                            @endforeach
                                        @else
                                            @foreach ($data->komitmen_mr->sasaran_t3 as $item)
                                                ...
                                                <tr>
                                                    ...
                                                    @foreach ($item->sasaran_list as $i)
                                                        @if ($loop->index == 0)
                                                            <td>{{ $i->isp_text . ' ' . number_format($i->isp_target, 3, ',', '.') . ' ' . $i->isp_satuan }}
                                                            </td>
                                                        @else
                                                </tr>
                                                <tr>
                                                        ...
                                                        <td>
                                                            ...
                                                            <ol>
                                                                @foreach ($itku->list as $il)
                                                                    <li>{{ ($il->kegiatan_tujuan ? $il->kegiatan_tujuan : $il->tujuan_kegiatan) . ' ' . number_format($il->tujuan_kegiatan, 3, ',', '.') . ' ' . $il->tujuan_kegiatan_satuan }}
                                                                    </li>
                                                                @endforeach
                                                            </ol>
                                                        </td>
                        ...
                        <div class="col-lg-12">
                            <button class="btn btn-default pull-right" id="btn-print-area-komitmen">Cetak</button>
                        </div>
                        <p style="page-break-after: always;">&nbsp;</p>
                        <div id="resiko">
                            <div class="col-lg-12">
                            ...
                            <div class="col-lg-12 table-responsive tableFixHead">
                                <table class="display table table-bordered table-hover" width="100%">
                                    <thead>
                                        <tr>
                                    ...
                                    <tbody>
                                        @foreach ($data->komitmen_mr->resiko->sortBy('resiko_prioritas') as $item)
                                            <tr>
                                                <td>{{ $item->resiko_kode . ($item->nama_pembina ? '' : '*') }}</td>
                                    ...
                                    <tfoot>
                                        <tr>
                                            <td colspan="29">
                                                ...
                                                <br>
                                                <br>
                                                <u>{{ $data->komitmen_mr->pemilik_resiko }}</u> <br>
                                                {{ $data->komitmen_mr->pemilik_resiko_nip }}
                                            </div> --}}
                                                <span>* Menunjukan risiko baru</span>
                                            </td>
                                        </tr>
                                    </tfoot>
                                </table>
                                <div class="col-sm-4 text-center pull-right">
                                    Jakarta, <?php echo date('d') . ' ' . AppHelper::getMonthIndo(date('m')) . ' ' . date('Y'); ?>
                                    <br>
                                    <br>
                                    <br>
                                    <u>{{ $data->komitmen_mr->pemilik_resiko }}</u> <br>
                        ...
                        <div class="col-lg-12">
                            <button class="btn btn-default pull-right" id="btn-print-area-resiko">Cetak</button>
                        </div>
                        <p style="page-break-after: always;">&nbsp;</p>
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-8">
                                    <table id="table-peta" class="table table-bordered" width="50%">
                                        <tr>
                                            <th colspan="{{ count($data->kriteria_dampak) + 3 }}" class="text-center">
                                        ...
                                        @foreach ($data->peta as $item)
                                            <tr style="height: 100px;">
                                                @if ($loop->index == 0)
                                                    <td rowspan="8"
                                                        style="width: 5%;vertical-align: middle;text-align: center;font-weight: bold;">
                                                        KEMUNGKINAN
                                                    </td>
                                                @endif
                                            ...
                                            @foreach ($data->kriteria_dampak as $val)
                                                @if ($val->nilai == 5)
                                                    <td
                                                        ...
                                                        {{ $val->nilai }}</td>
                                                @else
                                                    <td style="font-weight:bold;text-align: center;">{{ $val->nilai }}
                                                    </td>
                                                @endif
                                            @endforeach
                                        </tr>
                                    ...
                                    <table id="table-peta" class="table table-bordered" width="100%">
                                        <thead>
                                            ...
                                            <tr>
                                                ...
                                                <th colspan="3"
                                                    style="text-align: center; background-color: #001bf2; color: #fff;">
                                                    Nilai Risiko Yang Melekat</th>
                                                <th colspan="3"
                                                    style="text-align: center; background-color: #b600d4; color: #fff;">
                                                    Nilai Risiko Setelah Pengendalian</th>
                                                <th colspan="3"
                                                    style="text-align: center; background-color: #00bcd4; color: #000;">
                                                    Risiko Yang Direspon</th>
                                            </tr>
                                            <tr>
                                                @for ($i = 0; $i < 3; $i++)
                                                    @if ($i == 0)
                                        ...
                                        <tbody>
                                            @foreach ($data->komitmen_mr->resiko as $item)
                                                <tr>
                                                    ...
                                                    <td class="text-center">{{ $item->resiko_penilaian_kemungkinan }}
                                                    </td>
                                                    ...
                                                    <td class="text-center">
                                                        {{ $item->resiko_pengendalian_kemungkinan }}</td>
                                                    <td class="text-center">{{ $item->resiko_pengendalian_dampak }}
                                                    </td>
                                                    <td class="text-center"
                                                        style="background-color: rgb({{ $item->pr . ', ' . $item->pg . ', ' . $item->pb }})">
                                                        {{ $item->resiko_pengendalian_nilai }}</td>
                                                    <td class="text-center">
                                                        {{ $item->inovasi ? $item->inovasi->resiko_kemungkinan : '0' }}
                                                    </td>
                                                    <td class="text-center">
                        ...
                        <div class="col-lg-12">
                            <button class="btn btn-default pull-right" id="btn-simpan-pict-peta">Download Peta
                                Resiko</button>
                        </div>
                        <p style="page-break-after: always;">&nbsp;</p>
                                        ...
                                        <table id="table-jadwal" class="table table-bordered" width="100%">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th colspan="48" style="text-align: center">Tahun
                                                        {{ $keyyear }}</th>
                                                            ...
                                                            <tr>
                                                                ...
                                                                @for ($i = 1; $i <= 4; $i++)
                                                                    @for ($j = 1; $j <= 3; $j++)
                                                                        @if ($row->jenis == 'inovasi')
                                                                            @for ($k = 1; $k <= 4; $k++)
                                                                                @if ($i == implode('|', $row->resiko_triwulan) && $k == $week && $j == $bln)
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <button class="btn btn-default pull-right" id="btn-simpan-pict-jadwal">Download
                                    Jadwal</button>
                            </div>
                        </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            $.each(pointing, function(idx, val) {
                ...
                //     $('#' + val.inherent).append(
                //         '<div class="col-md-7">' + inherent +
                //         '<br></div>')
                //     $('#' + val.controlled).append(
                //         '<div class="col-md-7">' + controlled +
                //         '<br></div>')
                //     $('#' + val.respon).append(
                //         '<div class="col-md-7">' + respon + '<br></div>')
                // } else if (pointing.length >= 4 && pointing.length < 9) {
                //     $('#' + val.inherent).append(
                //         '<div class="col-md-5">' + inherent +
                //         '<br></div>')
                //     $('#' + val.controlled).append(
                //         '<div class="col-md-5">' + controlled +
                //         '<br></div>')
                //     $('#' + val.respon).append(
                //         '<div class="col-md-5">' + respon + '<br></div>')
                // } else if (pointing.length >= 9 && pointing.length < 16) {
                //     $('#' + val.inherent).append(
                //         '<div class="col-md-3">' + inherent +
                //          '<br></div>')
                //     $('#' + val.controlled).append(
                //         '<div class="col-md-3">' + controlled +
                //         '<br></div>')
                //     $('#' + val.respon).append(
                //         '<div class="col-md-3">' + respon + '<br></div>')
                // } else {
                $('#' + val.inherent).append(
            ...
            function ExportToExcel(type, fn, dl) {
                var elt = document.getElementById('resiko');
                ...
                return dl ?
                    ...
                    XLSX.writeFile(wb, fn || ('Peta-resiko-' + '{{ $data->komitmen_mr->mr_nomor }}' + '.' + (
                        type || 'xlsx')));
            }
            ...
            $('#btn-print-area-resiko').on('click', function() {
                ...
                // document.body.innerHTML = originalContents;
                ExportToExcel();
                // let file = new Blob([$('#resiko').html()], {type:"application/vnd.ms-excel"});
                // let url = URL.createObjectURL(file);
                // let a = $("<a />", {
                // href: url,
                // download: "Area-Resiko.xls"}).appendTo("body").get(0).click();
                // e.preventDefault();
            })
            ...
            //     // window.scrollTo(0,0);
            //     html2canvas(document.getElementById("take-pict-jadwal")).then(function(canvas) {
            //         var anchorTag = document.createElement("ab");
            //         document.body.appendChild(anchorTag);
            //         // document.getElementById("previewImg").appendChild(canvas);
            //         anchorTag.download = "jadwal-" + '{{ $data->komitmen_mr->mr_nomor }}' +
            //             ".jpg";
